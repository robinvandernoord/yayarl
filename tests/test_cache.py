...
def test_cache_configure_None_including_deprecated() -> None:
    ...
    assert yayarl.cache_info()["idna_decode"].maxsize is None
    assert yayarl.cache_info()["idna_encode"].maxsize is None
    assert yayarl.cache_info()["encode_host"].maxsize is None
...
def test_cache_configure_explicit() -> None:
    ...
    assert yayarl.cache_info()["idna_decode"].maxsize == 128
    assert yayarl.cache_info()["idna_encode"].maxsize == 128
    assert yayarl.cache_info()["encode_host"].maxsize == 128
...
def test_cache_configure_waring() -> None:
    ...
    assert yayarl.cache_info()["encode_host"].maxsize == 1024
    with pytest.warns(DeprecationWarning, match=msg):
        yayarl.cache_configure(host_validate_size=None)
